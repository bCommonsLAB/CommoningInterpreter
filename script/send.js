document.addEventListener("DOMContentLoaded", () => {
  ...
  function resetAll() {
    if (statusElement) statusElement.innerText = "Ready for analysis";
  ...
  sendBtn.addEventListener("click", () => {
    ...
    if (!userTextElement) {
      statusElement.innerText = "Error: Text input field not found";
      return;
    }
    const userText = userTextElement.value;

    if (userText.trim() === "") {
      statusElement.innerText = "Please enter a text.";
      return;
    }
    ...
    fetch("http://localhost:5001/analyze-text", {
      ...
      .then((data) => {
        hideLoading();
        if (data.error) {
          statusElement.innerText = "Error in analysis: " + data.error;
          ...
          statusElement.innerText = "Analysis completed";
        }
        showCenter();
      })
      .catch((error) => {
        hideLoading();
        statusElement.innerText = "Error in analysis";
        console.error(error);
      });
  });
